...
///
/// Cópia deliberada de <c>Clinica.Desktop/Servicos/CameraServico.cs</c>. A webcam está
/// no balcão, então quem precisa dela é a RECEPÇÃO; o shell não pode referenciar o
/// executável congelado do faturamento. Ver <c>docs/arquitetura-multi-exe.md</c>.
/// </summary>
public sealed class CameraServico : IDisposable
{
    ...
    public static IReadOnlyList<DispositivoCamera> Listar()
    {
        ...
        catch (Exception ex)
        {
            ...
            LogSuite.Registrar("Câmera — enumeração de dispositivos falhou", ex);
        }
        return lista;
    }
    ...
    public void Iniciar(DispositivoCamera camera)
    {
        ...
        {
            Parar();
            LogSuite.Registrar("Câmera — falha ao abrir o dispositivo", ex);
    ...
    public void Parar()
    {
        ...
        catch (Exception ex)
        {
            ...
            LogSuite.Registrar("Câmera — falha ao encerrar o dispositivo", ex);
        }
    }
    ...
    private void AoReceberQuadro(object sender, NewFrameEventArgs e)
    {
        ...
        {
            // Quadro corrompido: ignora e espera o próximo. É o único catch mudo do
            // projeto por desenho — isto roda 30x por segundo e logar entupiria o disco.
        }
    }
