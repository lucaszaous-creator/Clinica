...
public sealed partial class UsuarioEdicaoViewModel : ObservableObject
{
    ...
    private async Task CarregarCpfAsync()
    {
        ...
        {
            Clinica.Application.Diagnostico.Registrar("Faturamento — CPF do profissional não pôde ser lido", ex);
        }
    }
    ...
    private async Task CarregarAsync()
    {
        ...
        {
            Clinica.Application.Diagnostico.Registrar("Faturamento — cadastro de usuário não pôde ser aberto", ex);
    ...
    private async Task SalvarAsync()
    {
        try
        {
            // A segunda barreira, aqui mais que em qualquer tela: quem grava permissão
            // pode conceder permissão a si mesmo. A porta (Acessos) já exige o bit, mas
            // "só se chega por ali" foi exatamente a suposição que a parcela 54 derrubou.
        ...
        {
            Clinica.Application.Diagnostico.Registrar("Faturamento — usuário não pôde ser salvo", ex);
            Mensagem = ex.Message;
            MensagemEhErro = true;
        }
        finally
