...
public partial class ParametrosViewModel : ObservableObject, IAtalhosDeTela
{
    ...
    public ParametrosViewModel(IServiceScopeFactory scopeFactory, ISnackbarService snackbar, IDialogoService dialogo)
    {
        ...
        DadosClinica = new(scopeFactory, snackbar);
        RegrasFaturamento = new(scopeFactory, snackbar);
        _snackbar = snackbar;
        _dialogo = dialogo;
    }
    ...
    private async Task Salvar()
    {
        ...
        try
        {
            ...
            var prestadorAtual = await parametros.ObterPrestadorAsync();
            prestadorAtual.CodigosTuss = MontarPrestador().CodigosTuss;
            await parametros.SalvarPrestadorAsync(prestadorAtual);
