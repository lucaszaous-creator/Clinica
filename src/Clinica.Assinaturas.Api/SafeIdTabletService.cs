    ...
    AssinaturaDePrescricaoService assinadorInfusao)
{
    ...
    private (OpcoesSafeID Opcoes, Uri Retorno) Configuracao()
    {
        ...
        if(string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(segredo)
            ...
            || retorno.Scheme!="https" || retorno.Host!="portal.clinicasemdormacae.com.br"
            || retorno.AbsolutePath!="/safeid/retorno" || retorno.Query!="" || retorno.Fragment!="" || !retorno.IsDefaultPort)
    ...
    public async Task<object> IniciarAsync(SessaoTablet s,int agendamento,string tipo,int id,bool confirmou,CancellationToken ct)
    {
        ...
        var hash=await Conferir(tipo,id,confirmou,ct);
        ...
        return new {a.Id,Url=cliente.UrlDeAutorizacao(a.Pkce,retorno,EscopoSafeID.AssinaturaUnica,
            cpf:u.Profissional.Cpf,estado:a.Estado).AbsoluteUri};
    }
    public async Task<object> ConcluirAsync(SessaoTablet s,Guid id,CancellationToken ct)
    {
        ...
        try
        {
            ...
            if(db.Database.IsNpgsql()) await db.Database.ExecuteSqlInterpolatedAsync($"SELECT pg_advisory_xact_lock(20260916, {a.Agendamento})",ct);
            ...
            if(await Conferir(a.Tipo,a.Documento,a.ConfirmouAlergia,ct)!=a.ConteudoHash)
